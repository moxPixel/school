...
class CalendarRepository extends ServiceEntityRepository
{
    ...
    /**
     * Calcule du nombres de dates multiplier par le taux Jr de remuneration par mois pour l annéé en cour
     * @return int|mixed
     * @throws Exception
     * @throws \Doctrine\DBAL\Exception
     */
    public function totalRemunerationForTeacherByMonth($id): array
    {
        $date = new \DateTime();
        return $this->createQueryBuilder('c')
            ->addSelect('MONTH(c.start) as month , SUM(DATE_DIFF(c.end,c.start)) as total')
    ...
    /**
     * Calcule du nombres de dates multiplier par le taux Jr de remuneration pour le mois en cour
     * @return int|mixed
     * @throws Exception
     * @throws \Doctrine\DBAL\Exception
     */
    public function totalRemunerationForMonth($id): array
    {
        $date = new \DateTime();
        return $this->createQueryBuilder('c')
            ->addSelect('MONTH(c.start) as month , SUM(DATE_DIFF(c.end,c.start)) as total')
